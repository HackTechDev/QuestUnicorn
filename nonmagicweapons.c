...
void non_magic_weapons(char* name) {
  double mul;
  int option;
  double price;
  int item = 0;
  ...
  void show_top() {
    ...
    for (i=1; i<=11; i++) {
      gotoxy(2,i+3);
      sprintf(buffer,"%2d  %s",i,objects[i].name);
      txt_write(buffer);
      gotoxy(30,i+3); 
      sprintf(buffer,"%6.0f",objects[i].cost * mul);
      txt_write(buffer);
      }
    for (i=44; i<=45; i++) {
      gotoxy(42,i-40); 
      sprintf(buffer,"%2d  %s",i-32,objects[i].name);
      txt_write(buffer);
      gotoxy(70,i-40); 
      sprintf(buffer,"%6.0f",objects[i].cost * mul);
      txt_write(buffer);
    ...
    for (i=49; i<=53; i++) {
      gotoxy(42,i-43); 
      sprintf(buffer,"%2d  %s",i-35,objects[i].name);
      txt_write(buffer);
      gotoxy(70,i-43); 
      sprintf(buffer,"%6.0f",objects[i].cost * mul);
      txt_write(buffer);
  ...
  while (option != 0) {
    ...
    if (option>0) {
      if (option >=1 && option <= 11) item = option;
      if (option == 12) item = 44;
      if (option == 13) item = 45;
      if (option >=14 && option <= 19) item = option + 35;
