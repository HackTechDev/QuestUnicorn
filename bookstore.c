...
void bookstore(char* name) {
  ...
  void show_top() {
    ...
    for (i=1; i<=8; i++) {
      gotoxy(2,i+2);
      sprintf(buffer,"%2d  %s",i,objects[i + 136].name);
      txt_write(buffer);
      gotoxy(30,i+2);
      sprintf(buffer,"%6.0f",objects[i + 136].cost * mul);
      txt_write(buffer);
      }
    for (i=1; i<=7; i++) {
      gotoxy(42,i+2);
      sprintf(buffer,"%2d  %s",i+8,objects[i + 117].name);
      txt_write(buffer);
      gotoxy(70,i+2);
      sprintf(buffer,"%6.0f",objects[i + 117].cost * mul);
      txt_write(buffer);
      }
  ...
  while (option != 0) {
    ...
    if (option>0) {
      if (option < 9) item = option + 136;
      else item = option + 109;
