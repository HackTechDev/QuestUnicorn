...
void clothing_shop(char* name) {
  ...
  void show_top() {
    ...
    for (i=1; i<=6; i++) {
      gotoxy(22,i+2);
      sprintf(buffer,"%2d  %s",i,objects[i + 130].name);
      txt_write(buffer);
      gotoxy(50,i+2);
      sprintf(buffer,"%6.0f",objects[i + 130].cost * mul);
      txt_write(buffer);
      }
    for (i=7; i<=9; i++) {
      gotoxy(22,i+2);
      sprintf(buffer,"%2d  %s",i,objects[i + 142].name);
      txt_write(buffer);
      gotoxy(50,i+2);
      sprintf(buffer,"%6.0f",objects[i + 142].cost * mul);
      txt_write(buffer);
      }
  ...
  while (option != 0) {
    ...
    if (option>0) {
      if (option < 7) item = option + 130;
      else item = option + 142;
