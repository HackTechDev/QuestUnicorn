...
void jeweler(char* name) {
  ...
  void show_top() {
    ...
    for (i=1; i<=6; i++) {
      gotoxy(2,i+2);
      sprintf(buffer,"%2d  %s",i,objects[i + 124].name);
      txt_write(buffer);
      gotoxy(30,i+2);
      sprintf(buffer,"%6.0f",objects[i + 124].cost * mul);
      txt_write(buffer);
      }
    for (i=7; i<=11; i++) {
      gotoxy(2,i+2);
      sprintf(buffer,"%2d  %s",i,objects[i + 145].name);
      txt_write(buffer);
      gotoxy(30,i+2);
      sprintf(buffer,"%6.0f",objects[i + 145].cost * mul);
      txt_write(buffer);
      }
    for (i=1; i<=8; i++) {
      gotoxy(42,i+2);
      sprintf(buffer,"%2d  %s",i+11,objects[i + 95].name);
      txt_write(buffer);
      gotoxy(70,i+2);
      sprintf(buffer,"%6.0f",objects[i + 95].cost * mul);
      txt_write(buffer);
      }
  ...
  while (option != 0) {
    ...
    if (option>0) {
      if (option < 7) item = option + 124;
      else if (option < 12) item = option + 145;
      else item = option + 84;
